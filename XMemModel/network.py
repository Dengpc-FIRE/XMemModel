...
from XMemModel.aggregate import aggregate
from XMemModel.modules import *
from XMemModel.memory_util import *
...
class XMem(nn.Module):
    ...
    def encode_value(self, frame, image_feat_f16, h16, masks, is_deep_update=True): 
        # print(f"encode_value,frame.shape: {frame.shape}")
        # print(f"encode_value,image_feat_f16.shape: {image_feat_f16.shape}")
        # print(f"encode_value,h16.shape: {h16.shape}")
        # print(f"encode_value,masks.shape: {masks.shape}")
        ...
        # print(f"encode_value,g16:{g16.shape}")
        # print(f"encode_value,h16:{h16.shape}")
    ...
    def segment(self, multi_scale_features, memory_readout,
        ...
        # print(f"segment,multi_scale_features:{multi_scale_features.size}")
        # print(f"segment,memory_readout:{memory_readout.shape}")
        # print(f"segment,hidden_state:{hidden_state.shape}")

        hidden_state, logits = self.decoder(*multi_scale_features, hidden_state, memory_readout, h_out=h_out)
        prob = torch.sigmoid(logits)
        # print(f"segment,prob1:{prob.shape}")
        # if selector is not None:
        #     prob = prob * selector
        # print(f"segment,prob2:{prob.shape}")

        logits, prob = aggregate(prob, dim=1, return_logits=True)
        # print(f"segment,prob3:{prob.shape}")

        if strip_bg:
            # Strip away the background
            prob = prob[:, 1:]
        # print(f"segment,hidden_state:{hidden_state.shape}")
        # print(f"segment,logits:{logits.shape}")
        # print(f"segment,prob4:{prob.shape}")
