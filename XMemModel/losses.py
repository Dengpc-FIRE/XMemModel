...
class LossComputer:
    ...
    def compute(self, data, num_objects, it):
        ...
        # losses = defaultdict(lambda: torch.tensor(0., device=data['rgb'].device))


        b, t = data['rgb'].shape[:2]
        # print(f"[debug]:data['rgb']={data['rgb'].shape}")
        # print(f"{t}")
        losses['total_loss'] = 0
        # losses['total_loss'] = torch.tensor(0., device=data['rgb'].device)
        ...
        for ti in range(1, t):
            ...
            # print(f"[Debug]:losses:{losses['total_loss']}")
